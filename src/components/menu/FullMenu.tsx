...
import { BannerHeader, ListCard, AddOnBox } from './MenuComponents';
import { FlipFoodCard, FlipDrinkCard } from './FlipCard';
          ...
          return (
            <section key={section.id} id={section.id} className="mb-12 scroll-mt-32">
              ...
              {isListCardSection ? (
                ...
                // Grid layout for food/drinks with flip cards
                <div className="grid grid-cols-2 gap-4 px-4">
                  {section.items.map(item => {
                    if (isDrinkCardSection) {
                      return <FlipDrinkCard key={item.title} item={item} />;
                    }
                    return <FlipFoodCard key={item.title} item={item} />;
                  })}
                </div>
              )}
